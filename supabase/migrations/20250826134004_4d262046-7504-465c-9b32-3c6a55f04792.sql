...
ALTER TABLE public.profiles ADD COLUMN default_role app_role DEFAULT 'carer';
...
ALTER TABLE public.time_entries ADD COLUMN approved_by uuid;
ALTER TABLE public.time_entries ADD COLUMN approved_at timestamp with time zone;
ALTER TABLE public.time_entries ADD COLUMN status text DEFAULT 'pending' CHECK (status IN ('pending', 'approved', 'rejected'));
