...
CREATE POLICY IF NOT EXISTS "Family admins can insert shift schedules"
ON public.shift_schedules
FOR INSERT
WITH CHECK (
...
CREATE POLICY IF NOT EXISTS "Family admins can update shift schedules"
ON public.shift_schedules
FOR UPDATE
USING (
  has_family_role(auth.uid(), family_id, 'family_admin'::app_role)
...
CREATE POLICY IF NOT EXISTS "Family admins can delete shift schedules"
ON public.shift_schedules
FOR DELETE
USING (
  has_family_role(auth.uid(), family_id, 'family_admin'::app_role)
