-- Drop all policies and functions that depend on the enum
...
DROP POLICY IF EXISTS "Memberships: admin can update" ON public.user_memberships;
DROP POLICY IF EXISTS "Memberships: admin can delete" ON public.user_memberships;
DROP POLICY IF EXISTS "Care recipients: admin can insert" ON public.care_recipients;
DROP POLICY IF EXISTS "Care recipients: admin can update" ON public.care_recipients;
DROP POLICY IF EXISTS "Care recipients: admin can delete" ON public.care_recipients;
DROP POLICY IF EXISTS "Time entries: owners or admin can delete" ON public.time_entries;
DROP POLICY IF EXISTS "Notes: author or admin can update" ON public.care_notes;
DROP POLICY IF EXISTS "Notes: author or admin can delete" ON public.care_notes;
DROP POLICY IF EXISTS "Memberships: admin or creator can insert" ON public.user_memberships;
...
-- Drop functions
DROP FUNCTION IF EXISTS public.has_family_role(uuid, uuid, app_role);
DROP FUNCTION IF EXISTS public.is_member(uuid, uuid);

-- Drop table that uses the enum
DROP TABLE IF EXISTS public.user_memberships CASCADE;

-- Drop the default constraint on profiles
...
-- Recreate user_memberships table
