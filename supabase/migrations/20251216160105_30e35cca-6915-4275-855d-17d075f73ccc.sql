-- Drop and recreate the get_shift_instances_with_names function to include placeholder carers
...
AS $function$
  SELECT 
    ...
    sa.end_time,
    sa.carer_id,
    COALESCE(p.full_name, pc.full_name || ' (pending)', 'Unassigned') as carer_name,
    si.status,
    sa.shift_type,
